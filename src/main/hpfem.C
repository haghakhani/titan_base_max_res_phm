...
int main(int argc, char *argv[]) {
	...
	StatProps statprops;
	MatProps matprops(material_count, matnames, intfrictang, bedfrictang, porosity, mu, rho, epsilon,
	    gamma, frict_tiny, 1.0, 1.0, 1.0);
	TimeProps timeprops;
	...
	while (!(timeprops.ifend(0)) && !ifstop) {
		...
		if (timeprops.ifsave())
			saverun(&BT_Node_Ptr, myid, numprocs, &BT_Elem_Ptr, &matprops, &timeprops, &mapnames,
			    adaptflag, order_flag, &statprops, &discharge, &outline, &savefileflag);
	...
	saverun(&BT_Node_Ptr, myid, numprocs, &BT_Elem_Ptr, &matprops, &timeprops, &mapnames, adaptflag,
	    order_flag, &statprops, &discharge, &outline, &savefileflag);
	MPI_Barrier(MPI_COMM_WORLD);
	...
	if (numprocs > 1) {
		...
		MPI_Reduce(*(outline.pileheight), *(outline2.pileheight), NxNyout, MPI_DOUBLE, MPI_MAX, 0,
		    MPI_COMM_WORLD);
		MPI_Reduce(*(outline.max_kinergy), *(outline2.max_kinergy), NxNyout, MPI_DOUBLE, MPI_MAX, 0,
		    MPI_COMM_WORLD);
		MPI_Reduce(*(outline.cum_kinergy), *(outline2.cum_kinergy), NxNyout, MPI_DOUBLE, MPI_SUM, 0,
		    MPI_COMM_WORLD);
