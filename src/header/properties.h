...
struct MatProps {
	...
	MatProps(int material_countin, char **matnamesin, double intfrictin, double *bedfrictin,
			double porosityin, double muin, double rhoin, double epsilonin, double gammain,
...
//! number of cells in the x direction on the map
	...
	void output(MatProps* matprops_ptr, StatProps* statprops_ptr) {
		...
		fprintf(fp, "Nx=%d: X={%20.14g,%20.14g}\n"
				...
				xminmax[1] * matprops_ptr->LENGTH_SCALE, Ny, yminmax[0] * matprops_ptr->LENGTH_SCALE,
		...
		double ENERGY_SCALE = matprops_ptr->LENGTH_SCALE * matprops_ptr->GRAVITY_SCALE
